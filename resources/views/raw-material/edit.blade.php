    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-2xl border-t-4 border-gold-500">
                <div class="p-8">
                    <form action="{{ route('raw-material.update', $record) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <x-text-input id="tanggal_terima" name="tanggal_terima" type="date"
                                    class="mt-1 block w-full" :value="old('tanggal_terima', $record->tanggal_terima)"
                                    required />
                            ...
                            <div>
                                ...
                                <x-text-input id="supplier" name="supplier" type="text" class="mt-1 block w-full"
                                    :value="old('supplier', $record->supplier)" required />
                            ...
                            <div class="flex gap-2">
                                ...
                                <div class="w-1/3">
                                    ...
                                    <select id="satuan" name="satuan"
                                        ...
                                        <option value="Kg" {{ $record->satuan == 'Kg' ? 'selected' : '' }}>Kg</option>
                                        <option value="Ton" {{ $record->satuan == 'Ton' ? 'selected' : '' }}>Ton</option>
                                        <option value="Gram" {{ $record->satuan == 'Gram' ? 'selected' : '' }}>Gram
                                        </option>
                                    </select>
                                </div>
                            </div>
