    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-2xl border-t-4 border-gold-500">
                <div class="p-8">
                    <form action="{{ route('penjualan.update', $record) }}" method="POST" class="space-y-6">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <select id="status_bayar" name="status_bayar"
                                    ...
                                    <option value="unpaid" {{ $record->status_bayar == 'unpaid' ? 'selected' : '' }}>
                                        Unpaid (Belum Bayar)</option>
                                    <option value="partial" {{ $record->status_bayar == 'partial' ? 'selected' : '' }}>
                                        Partial (Sebagian)</option>
                                    <option value="paid" {{ $record->status_bayar == 'paid' ? 'selected' : '' }}>Paid
                                        (Lunas)</option>
                                </select>
                            </div>
                        </div>
