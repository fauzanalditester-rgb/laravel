                        ...
                        <table class="w-full text-left border-collapse">
                            <thead>
                                <tr class="bg-gray-50 text-gray-700 uppercase text-xs font-bold">
                                    ...
                                    <th class="p-3 border-b">Kreditur</th>
                            ...
                            <tbody>
                                @forelse ($records as $record)
                                    <tr class="hover:bg-gray-50 transition-colors text-sm">
                                        ...
                                        <td class="p-3 border-b font-bold">{{ $record->kreditur }}</td>
                                        <td class="p-3 border-b">{{ $record->keterangan }}</td>
                                        <td class="p-3 border-b text-right font-bold text-red-700">Rp
                                            {{ number_format($record->jumlah, 0, ',', '.') }}</td>
                                        <td class="p-3 border-b">
                                            <span
                                                class="px-2 py-1 rounded-full text-xs font-bold uppercase {{ $record->status == 'lunas' ? 'bg-green-100 text-green-700' : 'bg-gold-100 text-gold-700' }}">
                                                {{ $record->status }}
                                            </span>
                                        </td>
                                        <td class="p-3 border-b text-right space-x-2">
                                            <a href="{{ route('hutang.edit', $record) }}"
                                                class="text-gold-600 hover:text-gold-700 font-bold">Edit</a>
                                            <form action="{{ route('hutang.destroy', $record) }}" method="POST"
                                ...
                                @empty
                                    <tr>
                                        <td colspan="6" class="p-8 text-center text-gray-400 italic">Belum ada data hutang.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
